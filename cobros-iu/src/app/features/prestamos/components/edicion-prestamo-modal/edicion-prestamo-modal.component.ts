 ...
 * El cliente viene del contexto (no se puede cambiar en edición).
 */
@Component({
...
export class EdicionPrestamoModalComponent {
  ...
  // Datos del préstamo original (con cliente adjunto)
  ...
  // Computed: cantidad de cuotas derivada de total ÷ cuota (igual que RegistroPrestamoModal)
  ...
  fechaFinal = computed(() =>
    this.calcularFechaFinal(this.fechaPrestamo(), this.cantidadCuotas(), this.frecuenciaPago())
  );

  // Computed: valor de la última cuota (puede diferir si no divide exacto)
  valorUltimaCuota = computed(() => {
    const n = this.cantidadCuotas();
    return n > 0 ? this.valorTotal() - (n - 1) * this.valorCuota() : 0;
  });

  // Computed: ¿el último pago difiere del valor de cuota?
  descuadreExacto = computed(() => {
    const n = this.cantidadCuotas();
    return n > 0 && this.valorUltimaCuota() !== this.valorCuota();
  });

  // Computed: porcentaje de interés sobre lo prestado
  ...
  // Computed: fecha máxima para el date picker (hoy)
  fechaMaxima = computed(() => new Date().toISOString().split('T')[0]);
  ...
  // Computed: detección de cambios respecto al original
  hayDatosModificados = computed(() => {
    ...
    return (
      ...
      this.valorInteres()            !== original.interesProyectado ||
      this.valorCuota()              !== original.valorCuota ||
  ...
  errorCuotas = computed(() =>
    this.cantidadCuotas() <= 0
      ? 'Ingrese valor prestado, interés y cuota para calcular los períodos'
      : ''
  );
  ...
  private cargarDatosDelPrestamo(prestamo: PrestamoConCliente): void {
    ...
    this.valorInteres.set(prestamo.interesProyectado);
    this.valorCuota.set(prestamo.valorCuota);
    this.frecuenciaPago.set(prestamo.frecuenciaPago);
  }

  /**
   * Calcula la fecha final a partir de inicio, cantidad de períodos y frecuencia
   */
  ...
  /**
   * Cierra el modal con confirmación si hay cambios pendientes
   */
  cerrar(): void {
  ...
  onFechaPrestamoChange(value: string): void {
    if (value) this.fechaPrestamo.set(new Date(value));
  }
